...
export default function DashboardTopBar({
  ...
  return (
    <header
      className={cn(
        "flex h-[72px] shrink-0 items-center justify-between gap-4 border-b border-zg-border bg-zg-app px-4 transition-colors duration-200 ease-out md:px-8",
      )}
    >
      <div className="flex min-w-0 flex-1 items-center gap-3">
        <button
          type="button"
          className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-zg-fg transition-all duration-200 ease-out hover:bg-zg-surface md:hidden"
        ...
        <div className="min-w-0">
          <h1 className="truncate text-2xl font-bold tracking-tight text-zg-fg">{title}</h1>
          {subtitle ? <p className="mt-0.5 truncate text-sm text-zg-text-muted">{subtitle}</p> : null}
        </div>
      </div>
      ...
      <div className="flex shrink-0 items-center gap-2 sm:gap-3">
        ...
        <button
          type="button"
          className="relative inline-flex h-10 w-10 items-center justify-center rounded-full border border-zg-border bg-zg-surface text-zg-text-secondary transition-all duration-200 ease-out hover:border-zg-border-hover hover:text-zg-fg"
