...
export async function getActionImpacts(supabase: SupabaseClient, restaurantId: string): Promise<ActionImpact[]> {
  ...
  return (data ?? []).map((row) => ({
    ...
    beforeValue: typeof row.before_value === "number" ? row.before_value : null,
    afterValue: typeof row.after_value === "number" ? row.after_value : null,
    deltaAbsolute: typeof row.delta_absolute === "number" ? row.delta_absolute : null,
    deltaPercent: typeof row.delta_percent === "number" ? row.delta_percent : null,
